...
public class ClickablesContainersHandler : MainRefs
{
    ...
    public (ResourceType type, string id, bool canOpen) CheckInventoryConditions(IScriptableObjectData SOData)
    {
        ...
        foreach (var item in needList)
        {
            bool existResource = GetRef<AbstractSavingManager>().GetSavingData<InventorySavingData>().GetResourceTypeIDs(item.type).Contains(item.id);
    ...
    public void OnContainerOpened(AbstractContainer lootContainer)
    {
        ...
        if (param.Any())
        {
            GetRef<AbstractSavingManager>().GetSavingData<InventorySavingData>().RemoveResourceTypeID(param.First().value);
        }
    }
